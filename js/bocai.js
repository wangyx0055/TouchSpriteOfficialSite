$(document).ready(function(){
	...
	$(window).scroll(function(){
		if(ScollPostion().top > 200)    //监听是否需要加入滚动事件
		{
			menu_drop("scrollBar","block");
			buffer($id("scrollBar"),{"opacity":"100"});
		}else{
			buffer($id("scrollBar"),{"opacity":"0"},null,function(){
				menu_drop("scrollBar","none");
			});
		}
	});

	$id("scrollBar").onclick = function(){
		scroll_to_pos("top");
	}

	$("#scrollBar").hover(function(){
		$(this).css("backgroundPosition","0px -88px");
	},function(){
...
function scroll_to_pos(pos)
{
	...
	{
	    scrollY = $('body').height() - $(window).height() + 16;
	}
...
function  ScollPostion() {   //滚动条位置函数 返回JSON
    var t, l, w, h;
...
/*  
	潇洒菠菜 - easing API
	Quad Cubic Quart Quint 于自由落体运动差别不大 
	Sine 安全、均匀
	Expo 、 Circ 由慢变快
	Bounce 、Elastic 、Back 缓冲反弹等
	eg: easeInOutBounce
*/